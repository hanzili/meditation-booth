import { initNeurosity, disconnectNeurosity } from "./neurosityService.js";
import {
  initializePlayer,
  playMusic,
  stopMusic,
...
async function startSession(sessionId, musicName) {
  ...
  if (isSessionActive) {
    logger.warn("Attempt to start session while another is active", {
      sessionId,
    });
...
async function endSession(sessionId) {
  ...
  if (!isSessionActive || currentSessionId !== sessionId) {
    logger.warn("Attempt to end invalid or inactive session", {
      sessionId,
      currentSessionId,
      isSessionActive,
    });
  ...
  try {
    ...
    logger.info("Sending session end mutation", {
      sessionId,
